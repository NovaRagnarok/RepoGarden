...
- **Sleepy creatures have closed eyes.** Sprites whose vibe is `sleepy` (no commits in 14+ days) carve a 2-sub-pixel horizontal slit per eye instead of the dot-eye open variant. Same body footprint, same eye position — only the carving pattern changes, so a creature's silhouette stays stable when it dozes off. Visually reads as `▟▀▀▙` (closed) where an active sprite would render `▟▜▛▙` (open).
...
- `generateCreature` / `generateCreatureFrames` take an `eyesClosed` flag that propagates through `buildCreature` to a new `eyeCarvePositions` helper. Closed eyes append one inward sub-pixel per eye to the carve list; the cardinal-neighbour body-fill loop and the post-repair `reinforceEyes` pass both honour the extended list.
