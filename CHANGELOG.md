...
- **Sleepy creatures have closed eyes.** Sprites whose vibe is `sleepy` (no commits in 14+ days) render with `_` painted over each eye cell instead of the quadrant block char the body grid would otherwise produce there. The underlying sub-pixel body is untouched — vibe flips don't disturb silhouette or size. Visually reads as `▟__▙` (closed) where an active sprite would render `▟▜▛▙` (open).
