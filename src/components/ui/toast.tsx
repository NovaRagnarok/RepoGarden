...
export const Toast = ({
  ...
  // Only tick when we actually display the elapsed value. Otherwise the
  // 100 ms setState fires for the whole toast lifetime and Ink re-renders
  // every tick — which on the home screen visibly thrashes against the
  // direct-stdout star/sprite painters.
  useInterval(
    () => {
      setElapsed((e) => Math.min(e + TICK_MS, duration));
    },
    dismissed || !showProgress ? null : TICK_MS
  );
