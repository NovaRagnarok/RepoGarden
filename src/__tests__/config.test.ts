...
test("loadConfig returns v1 defaults when config is missing", () => {
  withFakeHome(() => {
...
test("loadConfig returns v1 defaults when config is malformed", () => {
  withFakeHome((home) => {
...
test("loadConfig sanitizes invalid legacy fields to v1 defaults", () => {
  withFakeHome((home) => {
    writeRawConfig(home, {
      themeId: 12,
...
test("updateConfig applies patches and preserves the v1 schema", () => {
  withFakeHome((home) => {
    writeRawConfig(home, {
      themeId: "monokai",
      scanRoots: ["/old"],
