...
// Extract approximate word count from any of the four styles produced by
// fakeName (kebab, PascalCase, camelCase, capitalised-kebab). Word boundaries
// are hyphens plus mid-string lowercase→uppercase transitions; a leading
// capital counts toward the first word, not a boundary.
const wordCountOf = (name: string): number => {
  let boundaries = 0;
  for (let i = 0; i < name.length; i += 1) {
    ...
    if (i > 0 && name[i] >= "A" && name[i] <= "Z" && name[i - 1] !== "-") {
      boundaries += 1;
    }
  }
  return boundaries + 1;
...
test("fakeName produces 1-3 words across the supported styles", () => {
  for (let i = 0; i < 50; i += 1) {
    const name = fakeName(`repo-${i}`);
    assert.match(name, /^[A-Za-z][A-Za-z-]*$/, `unexpected characters: ${name}`);
    const words = wordCountOf(name);
    assert.ok(words >= 1 && words <= 3, `unexpected word count for ${name}: ${words}`);
  }
});
...
test("fakeName word-count distribution roughly matches the 20/60/20 target", () => {
  ...
  for (let i = 0; i < total; i += 1) {
    const w = wordCountOf(fakeName(`sample-${i}`)) as 1 | 2 | 3;
    counts[w] += 1;
  }
...
test("fakeName covers each casing style across many ids", () => {
  let kebab = 0;
  let pascal = 0;
  let camel = 0;
  let kebabCap = 0;
  const total = 1000;
  for (let i = 0; i < total; i += 1) {
    const name = fakeName(`style-${i}`);
    const hasHyphen = name.includes("-");
    const firstUpper = name[0] >= "A" && name[0] <= "Z";
    const hasMidUpper = /[a-z][A-Z]/.test(name);
    if (hasHyphen && firstUpper) kebabCap += 1;
    else if (hasHyphen) kebab += 1;
    else if (firstUpper) pascal += 1;
    else if (hasMidUpper) camel += 1;
    else kebab += 1; // single lowercase noun, indistinguishable from kebab
  }
  // Each style should land at least once per ~250 samples — generous floor so
  // a stuck PRNG can't masquerade as variety.
  assert.ok(kebab > total * 0.2, `kebab bucket too small: ${kebab}`);
  assert.ok(pascal > total * 0.05, `pascal bucket too small: ${pascal}`);
  assert.ok(camel > total * 0.05, `camel bucket too small: ${camel}`);
  assert.ok(kebabCap > total * 0.02, `kebab-cap bucket too small: ${kebabCap}`);
});
...
test("redact(vibe) returns a per-vibe generic line", () => {
  ...
  assert.notEqual(redact("happy", "vibe"), redact("awake", "vibe"));
