...
<!-- shareable session snapshot — done: `export-text` headless subcommand scans once, renders the garden to text, and uses `--max-chars` / `--discord` to choose the widest fitting panorama across the bounded width range, with a fenced code block + project-URL footer for chat-share. See `runExportTextCli` in `src/lib/gif/cli.ts` and the dispatch in `src/cli-main.tsx`. -->
